...
class SearchEmails(Data):
    ...
    def search_emails_by_text(self):
        self.founded_emails = self.drop_email_duplicates()[
            self.drop_email_duplicates().str.contains(self.str_phrase)
        ]
    ...
    def __str__(self):
        return (
            f"Found emails with '{self.str_phrase}' in email ({self.count_founded_emails()}):\n\t"
